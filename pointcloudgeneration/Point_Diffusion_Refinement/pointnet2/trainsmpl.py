...
from util import training_smpl_loss, calc_diffusion_hyperparams
...
from models.pointnet2_with_smpl_condition import PointNet2SMPLCondition
...
def evaluate_per_rank(net, rank, num_gpus, root_directory, local_path, n_iter, 
    ...
    elif dataset in ['smpl_thuman_dataset', 'mvp40']:
        testloader = get_dataloader(trainset_config, phase=phase, rank=rank, world_size=num_gpus, 
...
def gather_eval_result_from_all_ranks(save_dir, n_iter, num_gpus, tb=None, test_trainset=False, dataset ='shapenet'):
    if not dataset in ['shapenet', 'shapenet_pytorch', 'smpl_thuman_dataset', 'shapenet_chunk', 'mvp40', 'partnet']:
    ...
    try:
        ...
        for f in files:
            ...
            elif dataset in ['smpl_thuman_dataset', 'shapenet_chunk', 'mvp40', 'partnet']:
        ...
        if dataset in ['smpl_thuman_dataset', 'shapenet_chunk', 'mvp40', 'partnet']:
...
def split_data(data, dataset, conditioned_on_cloud, include_class_condition, 
    ...
    if dataset in ['smpl_thuman_dataset', 'mvp40', 'partnet']:
        ...
        generated = data['coarse_pc'].cuda()[...,:3]
        # pcd = o3d.geometry.PointCloud()
        # pcd.points = o3d.utility.Vector3dVector(generated[0].squeeze().cpu().numpy())
        # o3d.io.write_point_cloud("test_generated.ply", pcd)
        # pcd = o3d.geometry.PointCloud()
        # pcd.points = o3d.utility.Vector3dVector(X[0].squeeze().cpu().numpy())
        # o3d.io.write_point_cloud("test_gt.ply", pcd)
...
def train(num_gpus, config_file, rank, group_name, dataset, root_directory, output_directory, 
    ...
    network_type = pointnet_config.get('network_type', 'pointnet++smpl')
    assert network_type in ['pointnet++smpl']
    ...
    elif network_type == 'pointnet++smpl':
        
        net = PointNet2SMPLCondition(pointnet_config).cuda()
    ...
    while n_iter < n_iters + 1:
        ...
        for data in trainloader: 
            ...
            if task == 'refine_completion':
                ...
                X, condition, label, smpl = split_data(data, dataset, conditioned_on_cloud, 
                                    pointnet_config.get('include_class_condition', False), 
                                    random_shuffle_points, task=task,
                                    random_subsample_partial_points=random_subsample_partial_points)
                if task == 'denoise':
            ...
            if task in ['refine_completion', 'denoise']:
                if task == 'refine_completion':
                    ...
                    if point_upsample_factor > 1:
                        ...
                        knn_index=pytorch3d.ops.knn_points(generated,generated,K=30,return_nn=True,return_sorted=True)[1]
                        knn_flow=pytorch3d.ops.knn_gather(displacement[...,:3], knn_index)
                        refined_X = generated + displacement[...,:3] 
                ...
                if cd_loss_type == 'cd_t':
                    ...
                    loss_idx = 0
                loss = clgd_loss(X, refined_X) + 10 * torch.mean(torch.square(displacement[...,:3].unsqueeze(2)-knn_flow[:,:,1:,:]))
                loss = loss.mean()
                ...
                loss = training_smpl_loss(net, loss_function, X, smpl, diffusion_hyperparams,
                                    label=None, condition=condition)
            ...
            if n_iter > 0 and (n_iter+1) % iters_per_ckpt == 0:
                ...
                if n_iter >= eval_start_iter and num_ckpts % eval_per_ckpt==0:
                    ...
                    elif dataset == 'smpl_thuman_dataset' or dataset == 'mvp40':
...
if __name__ == "__main__":
    ...
    if train_config['dataset'] == 'smpl_thuman_dataset':
        trainset_config = config["mvp_dataset_config"]
    elif train_config['dataset'] == 'refine_thuman_dataset':
        trainset_config = config['mvp_dataset_config']
