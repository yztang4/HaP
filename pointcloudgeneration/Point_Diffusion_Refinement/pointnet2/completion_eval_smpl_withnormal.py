...
from util import rescale, find_max_epoch, print_size, sampling_smpl, calc_diffusion_hyperparams, AverageMeter
from util_fastdpmv2 import fast_sampling_function_v2

torch_version = torch.__version__
if torch_version == '1.8.1+cu111':
...
def evaluate(net, testloader, diffusion_hyperparams, print_every_n_steps=200, parallel=True,
    ...
    if not dataset in ['smpl_thuman_dataset', 'refine_thuman_dataset', 'thuman_dataset', 'shapenet_chunk', 'mvp40', 'partnet']:
         raise Exception('%s dataset is not supported' % dataset)
    if use_a_precomputed_XT:
        assert task == 'completion'
        assert dataset in ['smpl_thuman_dataset', 'refine_thuman_dataset', 'mvp40', 'partnet'] # right now we only implemented this feature for mvp dataset
    if augment_data_during_generation:
        assert task == 'completion'
        assert dataset in ['smpl_thuman_dataset', 'refine_thuman_dataset', 'mvp40', 'partnet']
    if dataset == 'shapenet' or dataset=='shapenet_pytorch':
        total_meta = []
    elif dataset in ['smpl_thuman_dataset', 'refine_thuman_dataset', 'mvp40', 'partnet']:
    ...
    if save_generated_samples:
        ...
        if dataset in ['smpl_thuman_dataset', 'refine_thuman_dataset', 'mvp40', 'partnet']:
    ...
    for idx, data in enumerate(testloader):
        if dataset in ['smpl_thuman_dataset', 'mvp40', 'partnet']:
        ...
        elif dataset in ['refine_thuman_dataset']:
            label = data['label'].cuda()
            gt = data['gt_pc'].cuda()
        ...
        if task == 'refine_completion':
            ...
            displacement, predict_normal = net(generated, condition, ts=None, label=label)
            if point_upsample_factor > 1:
                ...
                predict_normal=F.normalize(predict_normal,dim=2)
                generated_data = torch.cat([generated_data,predict_normal],2)
            ...
            if save_multiple_t_slices:
                assert dataset in ['smpl_thuman_dataset', 'refine_thuman_dataset', 'mvp40', 'partnet']# shapenet is not supported yet
                generated_data, result_slices = sampling_smpl(net, (batch,num_points,3), 
                ...
                if fast_sampling:
                    generated_data = fast_sampling_function_v2(net, (batch,num_points,3), diffusion_hyperparams,  # DDPM parameters
                                diffusion_config,
                                print_every_n_steps=print_every_n_steps, label=label, 
                                verbose=False, condition=condition, smpl=smpl,
                                **fast_sampling_config)
                else:
                    # generated_data = gt + torch.normal(0, 0.01, size=gt.shape, device=gt.device)
                    generated_data = sampling_smpl(net, (batch,num_points,3), 
                                diffusion_hyperparams, 
                                print_every_n_steps=print_every_n_steps, label=label, 
                                condition=condition,
                                smpl=smpl,
        ...
        if torch_version == '1.8.1+cu111':
            if compute_cd:
                cd_p, dist, f1 = cd_module(generated_data[...,:3], gt[...,:3])
            ...
            if compute_emd and EMD_module_loaded:
                emd_cost = emd_module(generated_data[...,:3], gt[...,:3])
            else:
        ...
        if save_generated_samples:
            if dataset in ['shapenet', 'shapenet_pytorch']:
                ...
                if dataset == 'smpl_thuman_dataset':
                    save_file = os.path.join(save_dir, 'smpl_thuman_dataset_generated_data_%dpts.h5' % num_points)
                elif dataset == 'refine_thuman_dataset':
                    save_file = os.path.join(save_dir, 'refine_thuman_dataset_generated_data_%dpts.h5' % num_points)
                elif dataset == 'mvp40':
                ...
                if save_multiple_t_slices:
                    ...
                    for t in t_slices:
                        if dataset == 'smpl_thuman_dataset':
                            t_save_file = os.path.join(save_dir, 'smpl_thuman_dataset_generated_data_%dpts_T%d.h5' % (num_points, t))
                        elif dataset == 'refine_thuman_dataset':
                            t_save_file = os.path.join(save_dir, 'refine_thuman_dataset_generated_data_%dpts_T%d.h5' % (num_points, t))
                        elif dataset == 'mvp40':
