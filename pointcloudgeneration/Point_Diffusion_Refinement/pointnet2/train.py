...
def split_data(data, dataset, conditioned_on_cloud, include_class_condition, 
    ...
    if dataset in ['refine_thuman_dataset']:
        ...
        # generated = X+0.01*torch.randn_like(X).cuda()
    else:
        raise Exception('%s dataset is not supported' % dataset)
        ...
        for data in trainloader: 
            ...
            if task in ['refine_completion', 'denoise']:
                if task == 'refine_completion':
                    ...
                    displacement = net(generated, condition, ts=None, label=None)
                ...
                # gt_save = X[...,:3].clone().detach().cpu().numpy()
                # refine_save = refined_X.clone().detach().cpu().numpy()
                # condition_save = condition.clone().detach().cpu().numpy()
                # np.save("gt_save.npy",gt_save)
                # np.save("refine_save.npy",refine_save)
                # np.save("condition_save.npy",condition_save)
                ...
                # loss = 200*cdloss.mean()+normalloss
            ...
            if n_iter % iters_per_logging == 0:
                if task in ['refine_completion', 'denoise']:
                    ...
                    #     n_iter, reduced_loss, loss.item(), 200*cdloss.mean().item(), normalloss.item(), output_scale_factor_value, 
