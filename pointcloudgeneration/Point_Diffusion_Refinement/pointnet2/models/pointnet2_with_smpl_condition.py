...
# from pointnet2.models.HGFilter import HGFilter, index
import copy
import numpy as np
...
class PointNet2SMPLCondition(PointNet2SemSegSSG):
    ...
    def _build_model(self):
        ...
        partial_in_fea_dim = self.hparams['partial_in_fea_dim']
        ...
        if self.include_local_feature:
            ...
            print("partial_in_fea_dim is")
            print(partial_in_fea_dim)
    ...
    def forward(self, pointcloud, condition, smpl, ts=None, label=None, use_retained_condition_feature=False):
        r"""
        ...
        Bsm,Nsm,Csm = smpl.size()
        pad_smpl_zeros = torch.zeros(Bsm,Nsm,3,device=smpl.device, dtype=smpl.dtype)
        # smpl = torch.cat([smpl,pad_smpl_zeros],dim=2)
        condition = torch.cat([condition,smpl],dim=1)
