...
class SMPLTHUMAN(data.Dataset):
    def __init__(self, data_dir, train=True, npoints=2048, novel_input=True, novel_input_only=False,
        ...
        if train:
            ...
            self.input_path = '%s/mim_thuman_8ktrain.h5' % data_dir
        else:
            # self.input_path = '%s/thuman_testrgbpredsmpl2k.h5' % data_dir
            self.input_path = '%s/forreleasecode.h5' % data_dir
        ...
        if train:
            # idx = np.random.choice(18000,4000)
            # np.save("idx.npy",idx)
            ...
            self.gt_data  = np.array((input_file['all_points'][()]), dtype=np.float32)[:,:10000,:]
            self.smpl = np.array((input_file['smpl_points'][()]), dtype=np.float32)[:,:,:3]
