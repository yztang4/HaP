...
class RefineThuman(data.Dataset):
    def __init__(self, data_dir, train=True, npoints=2048, novel_input=True, novel_input_only=False,
        ...
        if train:
            self.input_path = '%s/thuman_8ktrain_refinen2.h5' % data_dir
        else:
            # self.input_path = '%s/thuman_testrgbpredsmpl2k.h5' % data_dir
            self.input_path = '%s/inthewild3normal.h5' % data_dir
        ...
        print('depth point clouds:', self.depth_pc.shape)
        # if not benchmark:
        print('gt point clouds:', self.gt_pc.shape)
        print('coarse point clouds:', self.coarse_pc.shape)
        ...
        self.len = self.depth_pc.shape[0]
    ...
    def __getitem__(self, index):
        ...
        result['depth_pc'] = copy.deepcopy(self.depth_pc[index])
        # if not self.benchmark:
        result['gt_pc'] = copy.deepcopy(self.gt_pc[index])
        result['coarse_pc'] = copy.deepcopy(self.coarse_pc[index])
...
if __name__ == '__main__':
    ...
    dataset = RefineThuman('/public/sdi/tangyingzhi/Project', train=False, npoints=2048, novel_input=True, novel_input_only=False,
