...
def sampling_smpl(net, size, diffusion_hyperparams, print_every_n_steps=100, label=0,
                verbose=True, condition=None, smpl=None, return_multiple_t_slices=False,
                t_slices=[5, 10, 20, 50, 100, 200, 400, 600, 800],
                use_a_precomputed_XT=False, step=100, XT=None):
    """
    Perform the complete sampling step according to p(x_0|x_T) = \prod_{t=1}^T p_{\theta}(x_{t-1}|x_t)
    ...
    with torch.no_grad():
        for t in range(start_iter, -1, -1): # t from T-1 to 0
            ...
            if condition is None:
                epsilon_theta = net(x, ts=diffusion_steps, label=label)  
            else:
                epsilon_theta = net(x, condition, smpl, ts=diffusion_steps, label=label, use_retained_condition_feature=True)
            if verbose:
                print('t %d epsilon_theta max %.2f min %.2f' % (t, epsilon_theta.max(), epsilon_theta.min()))
            sqrt_Alpha = torch.sqrt(Alpha[t])
            x = (x - (1-Alpha[t])/torch.sqrt(1-Alpha_bar[t]) * epsilon_theta) / sqrt_Alpha   
            if return_multiple_t_slices and t in t_slices:
                result_slices[t] = x 
            if t > 0:
...
def training_smpl_loss(net, loss_fn, X, smpl, diffusion_hyperparams, label=None, condition=None):
    """
    Compute the training loss of epsilon and epsilon_theta
    ...
    if condition is None:
        ...
        epsilon_theta = net(transformed_X, condition, smpl, ts=diffusion_steps.view(B,), label=label)
  
    sqrt_Alpha = torch.sqrt(Alpha[diffusion_steps])
    x_tm1 = (transformed_X - (1-Alpha[diffusion_steps])/torch.sqrt(1-Alpha_bar[diffusion_steps]) * epsilon_theta) / sqrt_Alpha  # update x_{t-1} to \mu_\theta(x_t)

    # if t > 0:
    x_tm1 = x_tm1 + Sigma[diffusion_steps] * std_normal(X.shape)  # add the variance term to x_{t-1} 
